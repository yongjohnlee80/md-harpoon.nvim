#!/usr/bin/env bash
# tests/run-all.sh — run every md-harpoon.nvim test suite and turn a
# silent abort into a loud failure. Enforces the family runner contract
# (see the shared convention `lua-nvim-plugin-development.md` § "The
# runner contract").
#
#   tests/run-all.sh          # every suite in tests/
#   tests/run-all.sh smoke    # one suite (basename, no .lua)
#
# The repo had no runner — tests/smoke.lua was invoked by hand from its
# header comment. This is the single entry point; it fails the run when a
# suite prints no summary (a silent mid-run abort), reports failures, or
# exits non-zero with no counted failures. New suites added to tests/ are
